...
H2_PHASE="${H2_PHASE:-timeline}"
RUN_ROOT="${RUN_ROOT:-runs/phase2/h2_timeline_4090_${H2_PHASE}_$(date +%Y%m%d_%H%M%S)}"
...
BW_SAMPLE_LIMIT="${BW_SAMPLE_LIMIT:-128}"
...
MODE_PRESET="${MODE_PRESET:-probe_freeze ppo_bw1e6}"
POINT_PRESET="${POINT_PRESET:-auto}"
DRY_RUN="${DRY_RUN:-0}"
...
if [[ "${POINT_PRESET}" == "auto" ]]; then
  POINTS=(
    "control_u050_to_u051|control|50|51|0"
    "control_u100_to_u101|control|100|101|0"
    "control_u150_to_u151|control|150|151|0"
    "control_u200_to_u201|control|200|201|0"
    "control_u250_to_u251|control|250|251|0"
    "control_u300_to_u301|control|300|301|0"
    "fixedtau_u200_to_u201|control|200|201|1"
    "fixedtau_u250_to_u251|fixedtau|250|251|1"
    "fixedtau_u275_to_u276|fixedtau|275|276|1"
    "fixedtau_u300_to_u301|fixedtau|300|301|1"
  )
else
...
function checkpoint_for() {
  ...
  if [[ "${base_update}" == "300" && -f "${base_run}/final.pt" ]]; then
    printf '%s\n' "${base_run}/final.pt"
    return
  fi
  printf '%s\n' "${ckpt}"
...
function mode_args() {
  ...
  case "${mode}" in
    probe_freeze)
      out_ref+=(--bw_actor_lr 0)
      ;;
    tiny_ppo)
      out_ref+=(--bw_actor_lr 3e-7)
      ;;
    ppo_bw1e6)
      out_ref+=(--bw_actor_lr 1e-6)
      ;;
    bwlocal_branch_h20)
      out_ref+=(
        --bw_actor_lr 1e-5
    ...
    bwlocal_mix05_h20)
      ...
      ;;
    *)
      echo "[ERROR] Unknown H2 timeline mode: ${mode}" >&2
      exit 1
      ;;
  esac
...
function run_case() {
  ...
  local ckpt
  ckpt="$(checkpoint_for "${base_run}" "${base_update}")"
  if [[ ! -f "${ckpt}" ]]; then
...
job_counter=0
for point_entry in "${POINTS[@]}"; do
  IFS='|' read -r case_name base_key base_update target_update fixed_tau <<< "${point_entry}"
...
echo "H2 timeline probe prepared in: ${RUN_ROOT}"
echo "Manifest: ${MANIFEST_FILE}"

if [[ "${DRY_RUN}" != "1" ]]; then
  "${PYTHON}" scripts/analysis/phase2/summarize_h2_probe.py \
    --run_root "${RUN_ROOT}" \
    --manifest "${MANIFEST_FILE}" \
    --out_csv "${RUN_ROOT}/h2_timeline_summary.csv" \
    --markdown | tee "${RUN_ROOT}/h2_timeline_summary.md"
fi
