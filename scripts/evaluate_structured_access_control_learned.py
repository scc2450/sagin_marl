...
def _load_model(path: Path, device: torch.device) -> tuple[AccessBidScorer, dict[str, Any]]:
    payload = torch.load(path, map_location=device, weights_only=False)
    hidden_dim = int(payload.get("hidden_dim", 64))
    model = AccessBidScorer(input_dim=len(pair_feature_names()), hidden_dim=hidden_dim).to(device)
        ...
        while len(rows) < int(episodes):
            ...
            if looks_like_driver_group(drivers):
                ...
                for snapshot_state in snapshot_states:
                    ...
                    assoc_pred, candidates_pred = predict_access_assoc_from_snapshot(
                        snapshot_state,
                        cfg,
                        model,
                        device,
                ...
                for local_slot, slot in enumerate(active_indices):
                    ...
                    base_assoc = np.asarray(snapshot_states[local_slot]["stage_assoc"], dtype=np.int32)
                    assoc_pred, candidates_pred = predict_access_assoc(
                        drivers[slot].env,
                        model,
                        device,
                        base_assoc=base_assoc,
                    ...
                    obs_new = [drivers[slot].env._get_obs(i) for i in range(len(drivers[slot].env.agents))]
                    bw_action = np.asarray(queue_aware_bw_policy(obs_new, cfg), dtype=np.float32)
                    step_result = drivers[slot].execute_stage_bw_and_step(bw_action)
...
def main() -> None:
    ...
    model, model_meta = _load_model(Path(args.model), device)
    ...
    payload = {
        ...
        "model_meta": {
            "hidden_dim": int(model_meta.get("hidden_dim", 64)),
            "bank": str(model_meta.get("bank", "")),
            "feature_names": list(model_meta.get("feature_names", [])),
        },
        "episodes": int(args.episodes),
