from __future__ import annotations

import argparse
import copy
import json
import os
import sys
from pathlib import Path
from typing import Any

ROOT = os.path.dirname(os.path.dirname(__file__))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

import numpy as np
import torch

from sagin_marl.env.config import load_config

from diagnose_structured_bw_fixed_teacher_fit import (
    _collect_panel_bank,
    _configure_train_scope,
    _make_learner,
    _masked_l1,
    _offline_fit_one_step,
    _safe_summary,
    _set_all_seeds,
)
from diagnose_structured_bw_fused_head_compare import _build_actor
from sagin_marl.rl.structured_mappo import _collate_dataclass, _index_dataclass


def _evaluate_first_actions_with_common_follow(
    *,
    cfg,
    follow_learner,
    snapshot_states: list[dict[str, Any]],
    first_actions_np: np.ndarray,
    target_actions_np: np.ndarray,
    valid_masks: np.ndarray,
) -> dict[str, Any]:
    returns_np = np.asarray(
        follow_learner._bw_clean_rollout_returns_parallel(
            snapshot_states=[dict(state or {}) for state in snapshot_states],
            first_actions=[
                np.asarray(first_actions_np[idx], dtype=np.float32).reshape(int(cfg.num_uav), int(cfg.users_obs_max))
                for idx in range(first_actions_np.shape[0])
            ],
        ),
        dtype=np.float32,
    )
    gap_np = _masked_l1(first_actions_np, target_actions_np, valid_masks)
    return {
        "return": _safe_summary(returns_np),
        "current_target_gap": _safe_summary(gap_np),
    }


def _lookup_fit(
    *,
    cfg,
    device: torch.device,
    follow_learner,
    snapshot_states: list[dict[str, Any]],
    target_actions_np: np.ndarray,
    valid_masks: np.ndarray,
    lookup_steps: int,
    lookup_lr: float,
    eval_schedule: list[int],
) -> tuple[list[dict[str, Any]], np.ndarray]:
    target_t = torch.as_tensor(target_actions_np, dtype=torch.float32, device=device)
    valid_t = torch.as_tensor(valid_masks, dtype=torch.bool, device=device)
    logits = torch.nn.Parameter(torch.zeros_like(target_t))
    optimizer = torch.optim.Adam([logits], lr=float(lookup_lr))

    def _masked_action() -> torch.Tensor:
        masked_logits = logits.masked_fill(~valid_t, float("-inf"))
        probs = torch.softmax(masked_logits, dim=-1)
        probs = torch.where(valid_t, probs, torch.zeros_like(probs))
        probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1.0e-8)
        return probs

    history: list[dict[str, Any]] = []
    for step in range(0, int(lookup_steps) + 1):
        with torch.no_grad():
            pred = _masked_action()
            pred_np = pred.detach().cpu().numpy().astype(np.float32, copy=False)
        if step in eval_schedule:
            metrics = _evaluate_first_actions_with_common_follow(
                cfg=cfg,
                follow_learner=follow_learner,
                snapshot_states=snapshot_states,
                first_actions_np=pred_np,
                target_actions_np=target_actions_np,
                valid_masks=valid_masks,
            )
            metrics["offline_step"] = int(step)
            history.append(
                metrics
            )
        if step >= int(lookup_steps):
            break
        optimizer.zero_grad()
        pred = _masked_action()
        mask_f = valid_t.to(dtype=pred.dtype)
        loss = torch.nn.functional.huber_loss(pred, target_t, reduction="none")
        loss = (loss * mask_f).sum() / mask_f.sum().clamp_min(1.0)
        loss.backward()
        optimizer.step()

    final_actions = _masked_action().detach().cpu().numpy().astype(np.float32, copy=False)
    return history, final_actions


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True)
    parser.add_argument("--actor_checkpoint", required=True)
    parser.add_argument("--panel_states", type=int, default=16)
    parser.add_argument("--panel_seed", type=int, default=35000)
    parser.add_argument("--teacher_horizon", type=int, default=5)
    parser.add_argument("--offline_steps", type=int, default=20)
    parser.add_argument("--lookup_steps", type=int, default=400)
    parser.add_argument("--actor_lr", type=float, default=1.0e-3)
    parser.add_argument("--lookup_lr", type=float, default=5.0e-2)
    parser.add_argument("--eval_steps", type=int, nargs="*", default=[0, 1, 2, 5, 10, 20])
    parser.add_argument("--shared_loc_readout", type=str, default="fused")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--json_out", type=str, default=None)
    args = parser.parse_args()

    cfg = load_config(args.config)
    setattr(cfg, "structured_bw_loc_readout", "fused")
    setattr(cfg, "bw_clean_per_user_loss", "huber")
    setattr(cfg, "bw_clean_grad_aggregation", "mean")
    device = torch.device(args.device)
    _set_all_seeds(int(getattr(cfg, "seed", 0) or 0))

    base_actor = _build_actor(cfg, args.actor_checkpoint, device)
    panel_rows, episode_count = _collect_panel_bank(
        cfg=cfg,
        actor=base_actor,
        device=device,
        panel_states=int(args.panel_states),
        seed_base=int(args.panel_seed),
    )
    snapshot_states = [dict(row["snapshot_state"] or {}) for row in panel_rows]
    local_states = [row["local_state"] for row in panel_rows]
    valid_masks = np.stack([np.asarray(row["valid_mask"], dtype=bool) for row in panel_rows], axis=0)
    fixed_ref_actions = np.stack([np.asarray(row["ref_action"], dtype=np.float32) for row in panel_rows], axis=0)

    base_follow_learner = _make_learner(cfg, base_actor, device, float(args.actor_lr))
    base_follow_learner.bw_clean_per_user_horizon = int(args.teacher_horizon)
    target_actions_np, rho_np, utility_np = base_follow_learner._bw_clean_target_actions_parallel(
        snapshot_states=[dict(state or {}) for state in snapshot_states],
        ref_actions=fixed_ref_actions,
        valid_masks=valid_masks,
    )
    ref_returns_np = np.asarray(
        getattr(base_follow_learner, "_bw_clean_last_ref_returns", np.zeros((len(snapshot_states),), dtype=np.float32)),
        dtype=np.float32,
    )
    target_returns_np = np.asarray(
        base_follow_learner._bw_clean_rollout_returns_parallel(
            snapshot_states=[dict(state or {}) for state in snapshot_states],
            first_actions=[
                np.asarray(target_actions_np[idx], dtype=np.float32).reshape(int(cfg.num_uav), int(cfg.users_obs_max))
                for idx in range(target_actions_np.shape[0])
            ],
        ),
        dtype=np.float32,
    )
    improving_mask = target_returns_np > (ref_returns_np + 1.0e-6)

    eval_schedule = sorted({int(step) for step in args.eval_steps if 0 <= int(step) <= int(args.offline_steps)})
    shared_cfg = copy.deepcopy(cfg)
    setattr(shared_cfg, "structured_bw_loc_readout", str(args.shared_loc_readout))
    shared_actor = _build_actor(shared_cfg, args.actor_checkpoint, device)
    train_scope_used = _configure_train_scope(shared_actor, "loc_head")
    shared_learner = _make_learner(shared_cfg, shared_actor, device, float(args.actor_lr))
    shared_learner.bw_clean_per_user_horizon = int(args.teacher_horizon)

    local_batch = _collate_dataclass(local_states, device)
    train_idx_np = np.flatnonzero(np.asarray(improving_mask, dtype=np.bool_))
    train_idx_t = torch.as_tensor(train_idx_np, dtype=torch.long, device=device)
    local_batch_train = _index_dataclass(local_batch, train_idx_t)
    target_actions_train_np = np.asarray(target_actions_np[train_idx_np], dtype=np.float32)
    valid_masks_train_np = np.asarray(valid_masks[train_idx_np], dtype=bool)
    target_actions_t = torch.as_tensor(target_actions_train_np, dtype=torch.float32, device=device)
    valid_masks_train = torch.as_tensor(valid_masks_train_np, dtype=torch.float32, device=device) > 0.5

    shared_history: list[dict[str, Any]] = []
    shared_final_actions_np = fixed_ref_actions.copy()
    for step in range(0, int(args.offline_steps) + 1):
        with torch.inference_mode():
            current_out = shared_learner.actor.act_bw(local_batch, deterministic=True)
        current_actions_np = np.asarray(current_out.action.detach().cpu().numpy(), dtype=np.float32)
        if step in eval_schedule:
            metrics = _evaluate_first_actions_with_common_follow(
                cfg=cfg,
                follow_learner=base_follow_learner,
                snapshot_states=snapshot_states,
                first_actions_np=current_actions_np,
                target_actions_np=target_actions_np,
                valid_masks=valid_masks,
            )
            metrics["offline_step"] = int(step)
            shared_history.append(metrics)
        shared_final_actions_np = current_actions_np
        if step >= int(args.offline_steps):
            break
        _offline_fit_one_step(
            learner=shared_learner,
            local_batch_train=local_batch_train,
            target_actions=target_actions_t,
            valid_masks_train=valid_masks_train,
        )

    lookup_history, lookup_actions_np = _lookup_fit(
        cfg=cfg,
        device=device,
        follow_learner=base_follow_learner,
        snapshot_states=snapshot_states,
        target_actions_np=target_actions_np,
        valid_masks=valid_masks,
        lookup_steps=int(args.lookup_steps),
        lookup_lr=float(args.lookup_lr),
        eval_schedule=eval_schedule,
    )

    result = {
        "config": str(Path(args.config).resolve()),
        "actor_checkpoint": str(Path(args.actor_checkpoint).resolve()),
        "panel": {
            "states": int(len(snapshot_states)),
            "episodes": int(episode_count),
            "seed_base": int(args.panel_seed),
        },
        "teacher_horizon": int(args.teacher_horizon),
        "fixed_bank": {
            "improving_frac": float(np.mean(improving_mask.astype(np.float32))) if improving_mask.size > 0 else 0.0,
            "train_samples": int(np.sum(improving_mask.astype(np.int32))),
            "ref_return": _safe_summary(ref_returns_np),
            "target_return": _safe_summary(target_returns_np),
            "target_minus_ref": _safe_summary(np.asarray(target_returns_np - ref_returns_np, dtype=np.float32)),
            "rho": _safe_summary(np.asarray(rho_np, dtype=np.float32)),
            "utility_l1": _safe_summary(np.asarray(utility_np, dtype=np.float32)),
        },
        "shared_student": {
            "loc_readout": str(args.shared_loc_readout),
            "train_scope_used": str(train_scope_used),
            "history": shared_history,
            "final": _evaluate_first_actions_with_common_follow(
                cfg=cfg,
                follow_learner=base_follow_learner,
                snapshot_states=snapshot_states,
                first_actions_np=shared_final_actions_np,
                target_actions_np=target_actions_np,
                valid_masks=valid_masks,
            ),
        },
        "lookup_student": {
            "lookup_steps": int(args.lookup_steps),
            "history": lookup_history,
            "final": _evaluate_first_actions_with_common_follow(
                cfg=cfg,
                follow_learner=base_follow_learner,
                snapshot_states=snapshot_states,
                first_actions_np=lookup_actions_np,
                target_actions_np=target_actions_np,
                valid_masks=valid_masks,
            ),
        },
    }

    json_text = json.dumps(result, ensure_ascii=False, indent=2)
    print(json_text)
    if args.json_out:
        out_path = Path(args.json_out)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json_text, encoding="utf-8")


if __name__ == "__main__":
    main()
