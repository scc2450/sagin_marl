...
修改为dpp策略，目标函数以及对应分数的计算修改为：

* **第一项（接入/流入收益）：**
    * **公式：** $\sum_{u}\sum_{g\in \mathcal S_u} [Q_g-Q_u]^+ x_{g,u}$
    * **代码：** `access_w * access_term`
    * **对应逻辑：** $[Q_g-Q_u]^+$ 就是代码里的 `gap`；$x_{g,u}$ 就是代码里分配带宽后的实际速率 `service`。把它们乘起来求和，就完全等同于 `access_term`。

* **第二项（回传/流出收益）：**
    * **公式：** $\sum_{u}\sum_{s\in \mathcal T_u} [Q_u-Q_s]^+ y_{u,s}$
    * **代码：** `backhaul_w * backhaul_term`
    * **对应逻辑：** 逻辑同上，无人机把数据传给卫星（或基站）赚取的压差红利。

* **第三项（动作惩罚/正则项）：**
    * **公式：** $-\lambda_a \sum_u |a_u|^2$
    * **代码：** `- reg_term`
    * **对应逻辑：** $\lambda_a$ 对应代码里的 `accel_cost`；$|a_u|^2$ 对应 `np.dot(cand, cand)`（即向量的内积，等于模的平方）。
