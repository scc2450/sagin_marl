...
Input is the x1p00 summary tree copied from the formal parameter sweep.  The
nominal point uses the same selected checkpoints, held-out seed bases, and
episode count as the Section 5 source-scenario comparison, while retaining
queue-layer and flow-decomposition metrics that were not registered in the
first compact main table.

Run with:

    /opt/homebrew/Caskroom/miniconda/base/envs/rl/bin/python \
        docs/paper/experiments/aggregate_phase4_nominal_rich_metrics_20260715.py
"""
...
SOURCE_DIR = TABLE_DIR / "phase4_nominal_selected_rich_summaries_20260715/x1p00"
